...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[poko-chat] userId=${userId?.slice(0, 8)} parkId=${parkId} trackedParkId=${trackedParkId} activeParkId=${activeParkId} mentionedPark=${mentionedParkId} msgs=${msgCount} hasAssistant=${hasAssistantMsgs} lastUser_len=${lastUserMsg?.length ?? 0}`);
    ...
    console.log(`[poko-chat] Live data fetched — weather: ${weather.slice(0, 80)} | alerts: ${alerts.slice(0, 80)} | scanner: ${scannerStatus}`);
    ...
    console.log(`[poko-chat] AI gateway response status=${response.status}`);
    ...
    console.error("poko-chat error:", e instanceof Error ? e.message : e, e instanceof Error ? e.stack : "");
    return new Response(
