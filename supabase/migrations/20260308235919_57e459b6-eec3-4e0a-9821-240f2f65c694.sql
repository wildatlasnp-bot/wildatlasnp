-- Update create_or_join_watch to clear orphaned_at when a watcher is added
...
BEGIN
  ...
  -- On conflict (existing target): elevate priority to high (2), reset next_check_at, and clear orphaned_at
  ...
  VALUES (p_park_id, p_permit_name, p_date_window_start, p_date_window_end, 'active', 2, now(), NULL)
  ON CONFLICT ON CONSTRAINT uq_scan_target DO UPDATE SET 
    ...
    orphaned_at = NULL  -- Clear orphaned timestamp when watcher added
