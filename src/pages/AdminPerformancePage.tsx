...
import { ArrowLeft, Trash2, Activity, Gauge, Zap } from "lucide-react";
import {
  type PerfEvent,
  getPerfEvents,
  subscribePerfEvents,
...
const AdminPerformancePage = () => {
  ...
  // Subscribe to live perf event updates
  ...
  const avgTabSwitch =
    tabSwitchEvents.length > 0
      ? Math.round(
          tabSwitchEvents.reduce(
            (sum, e) => sum + (e.data.duration_ms as number),
            0,
                  ...
                  {[...tabSwitchEvents].reverse().map((e, i) => (
                    <TableRow key={`ts-${i}`}>
                      ...
                      <TableCell className="text-right">
                        <Badge
                          variant={
                            (e.data.duration_ms as number) > 200
                              ? "destructive"
                              : "secondary"
                          }
                        >
